const InvoicesModule = {
            ...
            <tr>
                ...
                <td>
                        <div class="row-actions" style="gap: 6px;">
                            <a href="#facturas/${i.id}" class="btn-icon" style="width:28px;height:28px;" title="Ver Detalle"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg></a>
                            <button type="button" class="btn-icon" style="width:28px;height:28px;" onclick="InvoicesModule.printInvoice(${i.id}, 'thermal')" title="Imprimir Ticket Térmico (2Connect)"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9V2h12v7"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg></button>
                        ...
                        <div class="grid-2 mt-24">
                            ...
                            <div class="invoice-totals-wrapper">
                                <table class="invoice-totals-table" style="width:100%;border-collapse:collapse;">
                                    <tr><td style="padding:8px 0;text-align:left;color:var(--color-text-muted);font-size:13px;">Subtotal</td><td style="padding:8px 0;text-align:right;font-weight:500;font-size:13px;">${App.formatCurrency(inv.subtotal, inv.currency)}</td></tr>
                                    ${inv.discount_amount > 0 ? `<tr><td style="padding:8px 0;text-align:left;color:var(--color-text-muted);font-size:13px;">Descuento</td><td style="padding:8px 0;text-align:right;font-weight:500;font-size:13px;">-${App.formatCurrency(inv.discount_amount, inv.currency)}</td></tr>` : ''}
                                    ${inv.tax_amount > 0 ? `<tr><td style="padding:8px 0;text-align:left;color:var(--color-text-muted);font-size:13px;">ITBIS (${inv.tax_rate}%)</td><td style="padding:8px 0;text-align:right;font-weight:500;font-size:13px;">${App.formatCurrency(inv.tax_amount, inv.currency)}</td></tr>` : ''}
                                    <tr style="border-top:2px solid var(--color-border);"><td style="padding:12px 0;text-align:left;font-size:18px;font-weight:600;">Total</td><td style="padding:12px 0;text-align:right;font-size:18px;font-weight:700;color:var(--color-primary);">${App.formatCurrency(inv.total, inv.currency)}</td></tr>
                                    ${inv.amount_paid > 0 ? `<tr><td style="padding:8px 0;text-align:left;color:var(--color-text-muted);font-size:13px;">Pagado</td><td style="padding:8px 0;text-align:right;font-weight:500;color:var(--color-success-icon);font-size:13px;">-${App.formatCurrency(inv.amount_paid, inv.currency)}</td></tr>` : ''}
                                    ${inv.status !== 'paid' ? `<tr><td style="padding:8px 0;text-align:left;font-weight:600;font-size:13px;">Balance</td><td style="padding:8px 0;text-align:right;font-weight:600;color:var(--color-danger-icon);font-size:13px;">${App.formatCurrency(inv.total - inv.amount_paid, inv.currency)}</td></tr>` : ''}
                                </table>
                            </div>
                        </div>
                    </div>
    ...
    async renderForm(container, editId = null) {
        ...
        document.getElementById('invoice-form').addEventListener('submit', async (e) => {
            ...
            const payload = {
                ...
                discount_value: parseFloat(document.getElementById('i_discount')?.value) || 0,
                tax_rate: parseFloat(document.getElementById('i_tax')?.value) || 0,
    ...
    renderItems() {
        ...
        container.innerHTML = this.items.map(item => `
            <div class="invoice-item-row">
                <div class="invoice-item-desc"><input type="text" id="item_desc_${item.id}" list="catalog_items_list" class="form-control" placeholder="Descripción del concepto..." required oninput="InvoicesModule.onItemDescChange(${item.id})"></div>
                <div class="invoice-item-qty"><input type="number" id="item_qty_${item.id}" class="form-control" placeholder="Cant." min="0.01" step="0.01" value="${item.qty || 1}" required oninput="InvoicesModule.calculateTotals()"></div>
                <div class="invoice-item-price"><input type="number" id="item_price_${item.id}" class="form-control" placeholder="Precio" min="0" step="0.01" value="${item.price || ''}" required oninput="InvoicesModule.calculateTotals()"></div>
                <button type="button" class="btn-icon invoice-item-delete" style="color:var(--color-danger-icon);" onclick="InvoicesModule.removeItem(${item.id})">
    ...
    async printInvoice(id, type = 'thermal') {
        ...
        let template = 'normal';
        if (type === 'thermal') {
            template = settings.invoice_pdf_template && settings.invoice_pdf_template.startsWith('thermal') 
