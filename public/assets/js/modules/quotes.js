        ...
        try {
            ...
            container.innerHTML = `
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:var(--spacing-lg);flex-wrap:wrap;gap:12px;">
                    ...
                    <div class="segmented-control" id="qt-status-tabs" style="margin-bottom:0;">
    ...
    renderItems() {
        ...
        c.innerHTML = this.items.map(item => `
            <div class="invoice-item-row">
                <div class="invoice-item-desc"><input type="text" id="q_item_desc_${item.id}" list="catalog_items_list" class="form-control" placeholder="Descripción..." required oninput="QuotesModule.onItemDescChange(${item.id})"></div>
                <div class="invoice-item-qty"><input type="number" id="q_item_qty_${item.id}" class="form-control" placeholder="Cant." min="0.01" step="0.01" value="1" required oninput="QuotesModule.calculateTotals()"></div>
                <div class="invoice-item-price"><input type="number" id="q_item_price_${item.id}" class="form-control" placeholder="Precio" min="0" step="0.01" required oninput="QuotesModule.calculateTotals()"></div>
                <button type="button" class="btn-icon invoice-item-delete" style="color:var(--color-danger-icon);" onclick="QuotesModule.removeItem(${item.id})"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>
            </div>
        `).join('');
