...
<head>
    ...
    <title>Recordatorio de Pago: Factura <?= htmlspecialchars($invoice['invoice_number']) ?></title>
    <style>
        ...
        .header { background-color: #F59E0B; padding: 30px 40px; text-align: center; color: #ffffff; }
        .content { padding: 40px; color: #333333; line-height: 1.6; font-size: 15px; }
        .footer { text-align: center; padding: 20px; color: #94A3B8; font-size: 12px; }
        .invoice-details { background-color: #FFFBEB; padding: 20px; border-radius: 6px; margin: 20px 0; border: 1px solid #FCD34D; }
        .amount { font-size: 24px; font-weight: bold; color: #D97706; margin: 10px 0; }
    </style>
</head>
<body>
                ...
                <table class="main" cellpadding="0" cellspacing="0" role="presentation">
                    ...
                    <tr>
                        <td class="content">
                            ...
                            <p>Este es un recordatorio de que la factura <strong><?= htmlspecialchars($invoice['invoice_number']) ?></strong> vencerá pronto o ya se encuentra vencida.</p>
                            
                            <div class="invoice-details">
                                <p style="margin: 0 0 5px 0; color: #92400E; font-size: 13px; text-transform: uppercase;">Balance Pendiente</p>
                                <div class="amount"><?= htmlspecialchars($invoice['currency']) ?> <?= number_format($invoice['total'] - $invoice['amount_paid'], 2) ?></div>
                                <p style="margin: 5px 0 0 0; color: #92400E; font-size: 13px;">Fecha de Vencimiento: <strong><?= date('d/m/Y', strtotime($invoice['due_date'])) ?></strong></p>
                            </div>

                            <p>Una copia de la factura está adjunta para su referencia.</p>
                            <p>Si ya has realizado este pago, por favor ignora este correo.</p>
